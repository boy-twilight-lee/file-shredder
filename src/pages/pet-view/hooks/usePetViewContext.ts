...
// 定义允许通过点击空白处直接关闭的气泡页面，任务执行期间的气泡不在此列。
const DISMISSIBLE_BUBBLE_MODES: PetBubbleMode[] = [
  'actions',
  'settings',
  'records',
];
...
export function usePetViewContext() {
  return {
    ...
    provide(): PetViewContext {
      ...
      // 判断当前气泡页面是否允许通过点击空白处关闭。
      function isDismissibleBubble(): boolean {
        return DISMISSIBLE_BUBBLE_MODES.includes(bubbleMode.value);
      }
      // 点击气泡与传送浮层之外的空白区域时关闭当前气泡。
      function handleOutsidePointerDown(event: PointerEvent): void {
        if (event.button !== 0 || !isDismissibleBubble()) return;
      ...
      // 透明窗口失焦时补充处理气泡之外的空白区域点击。
      function handleWindowBlur(): void {
        // 任务执行期间需要持续展示进度，其余页面失焦后立即收起。
        if (isDismissibleBubble()) closeBubble();
      }
      // 将任务级错误转换为可展示的失败结果。
    ...
    inject(): PetViewContext {
      return inject(PET_VIEW_CONTEXT_KEY, {
        ...
        // 在上下文外回退到内置形象的真实高宽比。
        petAspectRatio: ref(PET_VIEW_DEFAULTS.petAspectRatio),
        petMotion: ref(null),
