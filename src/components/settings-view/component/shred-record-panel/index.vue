...
const paginatedLogs = computed(() => {
  ...
  return props.logs.slice(startIndex, startIndex + recordPageSize.value);
});
// Arco 将 total=0 计算为 0 页；渲染总数至少为 1，以便空状态仍展示第 1 页。
const paginationRenderTotal = computed(() => Math.max(props.logs.length, 1));

function formatLogTime(timestamp: string): string {
  return new Date(timestamp).toLocaleString();
}

watch([() => props.logs.length, recordPageSize], ([logCount]) => {
