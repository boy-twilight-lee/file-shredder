...
contextBridge.exposeInMainWorld('shredderApi', {
  ...
  onPetComplete: (callback: (summary: unknown) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, summary: unknown) => callback(summary);
  ...
  onPetPlacement: (callback: (placement: string) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, placement: string) => callback(placement);
