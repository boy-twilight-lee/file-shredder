...
当前实现使用 `src/assets/imgs/` 下的透明 WebP。资源目录只保留运行时图片且不再分子目录，`docs/pet-assets/` 保留可重建源图，检查报告与预览统一输出到 Git 忽略的 `output/pet-assets/`。

| 状态                   | 画面                                   | 播放方式                                                         |
| ---------------------- | -------------------------------------- | ---------------------------------------------------------------- |
| 默认                   | 微笑、眨眼，身体保持不动               | 24fps 时间基准，5 秒循环、120 个时间采样；相同停留帧由 WebP 合并 |
| 操作菜单 / 设置 / 记录 | 手指向气泡所在的一侧                   | 固定图片；气泡在人物右侧时由渲染层镜像同一张素材                 |
...
| 取消                   | 使用默认微笑眨眼姿势                   | 不再存在独立 review 状态或素材                                   |
...
`review` 已从 `PetPose`、资源映射和业务分支中移除，`review.webp` 同步删除。取消任务回落到 idle；操作、设置与记录面板共用 `actions.webp`，人物手指始终指向气泡：气泡在人物左侧时使用素材本身的朝左姿势，气泡在人物右侧时由渲染层对同一张素材做 `scaleX(-1)` 镜像，因此不再维护一份重复的镜像图片。确认、工作、成功和失败仍分别使用自己的业务动作。

`actions.webp` 为 640×640 正方形画布，人物头部中心与身体在画布中接近水平居中，整幅镜像后人物主体位置基本不变，只有伸出的手臂换到另一侧，因此镜像不需要重新锚定。

内置默认形象（`PetImageService` 的 `built-in-ao-yin`）直接复用 `idle.webp`：主进程用它判定默认形象并生成设置页缩略图，渲染进程用它作为首次解码前的占位画面，因此不再维护独立的静态预览图。由于 `nativeImage` 无法解码动画 WebP，内置形象的原始尺寸由渲染进程上报的 640×640 校准。
